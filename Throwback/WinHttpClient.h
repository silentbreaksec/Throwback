...
public:
	...
	inline bool SetProxyUrl(const wstring &proxyUrl);
...
/**
WinHttpClient::WinHttpClient(int a)
{
}
**/
    ...
    if (::WinHttpCrackUrl(m_requestURL.c_str(), m_requestURL.size(), 0, &urlComp))
    {
        ...
        if (hConnect != NULL)
        {
            ...
            if (hRequest != NULL)
            {
				...
				//WinHttpSetCredentials(hRequest, WINHTTP_AUTH_TARGET_PROXY, WINHTTP_AUTH_SCHEME_NEGOTIATE, L"", L"", NULL);
				WinHttpSetOption(hRequest, WINHTTP_OPTION_AUTOLOGON_POLICY, (LPVOID)&autologon, sizeof(DWORD));
                ...
                while (!bGetReponseSucceed && iRetryTimes++ < INT_RETRYTIMES)
                {
					...
					if (m_proxyUrl.size() < 1)
                    {
						if(::WinHttpSendRequest(hRequest, WINHTTP_NO_ADDITIONAL_HEADERS, 0, WINHTTP_NO_REQUEST_DATA, 0, 0, NULL)) bSendRequestSucceed = true;
                    }
                    else
                    {
...
bool WinHttpClient::SaveResponseToFile(const wstring &filePath)
{
    ...
    {
		int i = 0;
        i = fwrite(m_pResponse, m_responseByteCountReceived, 1, f);
        i = fclose(f);
        return true;
    }
...
bool WinHttpClient::SetProxyUrl(const wstring &proxyUrl)
{
    m_proxyUrl = proxyUrl;
